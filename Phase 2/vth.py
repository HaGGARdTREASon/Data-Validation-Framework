...
# Dynamically find the length and Vth sensitivity columns
...
train_vth_col = [c for c in df_train.columns if 'vth' in c.lower() and 'sens' in c.lower()][0]
test_vth_col = [c for c in df_test.columns if 'vth' in c.lower() and 'sens' in c.lower()][0]

df_train = df_train.dropna(subset=[train_len_col, train_vth_col])
df_test = df_test.dropna(subset=[test_len_col, test_vth_col]).sort_values(by=test_len_col)

X_train, X_test = df_train[[train_len_col]].values, df_test[[test_len_col]].values
y_train_phys = df_train[train_vth_col].values
sim_len, sim_sens_phys = df_test[test_len_col].values, df_test[test_vth_col].values
...
r2_vth = r2_score(sim_sens_scaled, y_pred_calibrated)
mse_vth = mean_squared_error(sim_sens_scaled, y_pred_calibrated)
rmse_vth = np.sqrt(mse_vth)
mae_vth = mean_absolute_error(sim_sens_scaled, y_pred_calibrated)
...
print("========================================")
print("  CAVITY LENGTH VS. VTH SENSITIVITY METRICS ")
print("========================================")
print(f"R2 Score: {r2_vth:.6f}")
print(f"MSE:      {mse_vth:.8f} (Normalized)")
print(f"RMSE:     {rmse_vth:.8f} (Normalized)")
print(f"MAE:      {mae_vth:.8f} (Normalized)")
...
MAX_DEV = 0.05  # STRICT OUTLIER THRESHOLD (Only keep if deviation is <= 5%)
...
for i in range(len(df_test)):
    ...
    if not matches.empty:
        matches['scaled_sens'] = scaler.transform(matches[train_vth_col].values.reshape(-1, 1)).ravel()
        ...
        if deviations.min() <= MAX_DEV:
            ...
            final_plot_sens_phys.append(best_point[train_vth_col])
...
plt.ylabel('Vth Sensitivity')
plt.title('Cavity Length vs Vth Sensitivity')
