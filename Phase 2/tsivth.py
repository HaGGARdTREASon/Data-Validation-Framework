...
# Dynamically find the Vth sensitivity column names (handles 'Vth_sensitivity' or 'Vth sensitivity')
...
df_train = df_train.dropna(subset=['Tsi', train_vth_col])
df_test = df_test.dropna(subset=['Tsi', test_vth_col]).sort_values(by='Tsi')

X_train, X_test = df_train[['Tsi']].values, df_test[['Tsi']].values
# Extract values as flat 1D arrays for standard training
y_train_phys = df_train[train_vth_col].values.ravel()
y_test_phys = df_test[test_vth_col].values.ravel()
...
r2_vth = r2_score(y_test_phys, y_pred_phys)
mse_vth = mean_squared_error(y_test_phys, y_pred_phys)
rmse_vth = np.sqrt(mse_vth)
mae_vth = mean_absolute_error(y_test_phys, y_pred_phys)

# Calculate physical percentage Accuracy
acc_vth = 100 - (np.mean(np.abs((y_test_phys - y_pred_phys) / y_test_phys)) * 100)

print("========================================")
print("     TSI VS. VTH SENSITIVITY METRICS    ")
print("========================================")
print(f"R2 Score: {r2_vth:.6f}")
print(f"MSE:      {mse_vth:.8f}")
print(f"RMSE:     {rmse_vth:.8f}")
print(f"MAE:      {mae_vth:.8f}")
print(f"Accuracy: {acc_vth:.4f}%")
print("========================================\n")
...
plt.title('Tsi vs Vth Sensitivity')
