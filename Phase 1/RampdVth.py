...
df_train = pd.read_excel(train_file).dropna(subset=['Width', 'Vth sensitivity'])
df_test = pd.read_excel(test_file).dropna(subset=['Width', 'Vth sensitivity']).sort_values(by='Width')

X_train, X_test = df_train[['Width']].values, df_test[['Width']].values
y_train_phys = df_train['Vth sensitivity'].values.reshape(-1, 1)
y_test_phys = df_test['Vth sensitivity'].values.reshape(-1, 1)
...
plt.title('Rampdown: Width vs Vth Sensitivity')
plt.legend()
plt.grid(True)
plt.show()